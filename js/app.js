  ...
  return `
    <article class="exercise">
      ...
      <div class="alternative-box">
        <label class="alternative-label" for="alternative-${index}">
          Alternative exercise used this week
        </label>
        <input
          ...
          type="text"
          placeholder="Example: DB Bench Press"
        />
      </div>
...
function collectExerciseInputs(index) {
  ...
  const alternativeInput = $(`input[data-exercise="${index}"][data-type="alternative"]`);
  const alternative = alternativeInput ? alternativeInput.value.trim() : '';

  return { weights, reps, alternative };
}
...
function loadInputs() {
  ...
  day.exercises.forEach((exercise, index) => {
    ...
    const alternativeInput = $(`input[data-exercise="${index}"][data-type="alternative"]`);
    if (alternativeInput) {
      alternativeInput.value = saved.alternative || '';
    }
  });
}
...
function saveWorkout() {
  ...
  day.exercises.forEach((exercise, index) => {
    ...
    const { weights, reps, alternative } = collectExerciseInputs(index);

    if (!weights.length && !reps.length && !alternative) return;
  ...
  for (let week = 1; week <= 16; week += 1) {
    ...
    Object.keys(sessions[week]).sort().forEach((dayIndex) => {
      ...
      Object.values(sessions[week][dayIndex]).forEach((entry) => {
        const exerciseLabel = entry.alternative
          ? `${entry.name} → ${entry.alternative}`
          : entry.name;

        html += `<span class="pill">${exerciseLabel}: ${entry.summary}</span>`;
      });
...
function exportData() {
  const data = {
    ...
    version: 'clean-structured-v3-alternative-exercise',
